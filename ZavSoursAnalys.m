function ZavSoursAnalys(flNm, proc, isStim, etPrgs, chanMask, chnlGrp, nlxVer, muacr, dwnSmplFrq)
%ZavSoursAnalys(flNm, proc, isStim, etPrgs, chanMask, chnlGrp, nlxVer, muacr, dwnSmplFrq)
...
%nlxVer - version ov NLX data (0 - initial (Cheetah 5.4.0), 1 - new (Cheetah 6.3.2))
%muacr - common reference for MUA calculation
%dwnSmplFrq - downsampled-discretization frequency (Hz, for signals to be treated)
%
%OUTPUTS
...
if all(exist(flNm, 'file') ~= [2, 7])
    ...
    if (all(flNm == 0) || all(pth == 0)) %no filename assigned
        return;
    end
end

%define slash
z = find(flNm == '\', 1, 'last');%find slash
if isempty(z)
...
if (exist(flNm, 'dir') == 7) %directory (Neuralynx)
    if (flNm(end) ~= slashStr) %no slash
        flNm(end + 1) = slashStr;
    end
    svFlNm = [flNm(1:(end - 1)), '.mat'];%pathname of file for calculated values
else %'.abf' or '.edf' or '.daq' or '.oebin'
    z = find(flNm == '.', 1, 'last');
    if isequal(flNm((z + 1):end), 'oebin') %OpenEphys files
        z = find(flNm == slashStr, 1, 'last');
        svFlNm = [flNm(1:(z - 1)), '.mat'];%pathname of file for calculated values
    else
        svFlNm = [flNm(1:z), 'mat'];%pathname of file for calculated values
    end
end
zavp = struct('file', [], 'siS', [], 'dwnSmplFrq', [], 'stimCh', [], 'realStim', [], 'rarStep', []);%structure with parameters
zavp.file = flNm;%file path and name

[~, hd, synchro] = ZavLoadData(flNm, [], 1, 1, [], [], nlxVer);%load header of file
largestF = [find(isfinite(hd.recChDurations), 1, 'first'), find(isfinite(hd.ch_si), 1, 'first')];%largest recordation and smallest discretization time
%largestF(1) - number of file (in ncsFiles structure) with largest recordation
%largestF(2) - number of file (in ncsFiles structure) with smallest discretization time (hd.si and hd.ch_si(largestF(2)) are equal)
if ((exist(flNm, 'dir') == 7) && ~strcmp(bufStr, '.abf') && ~strcmp(bufStr, '.edf') && ~strcmp(bufStr, '.daq')) %Neuralynx-files
    for ch = 1:hd.nADCNumChannels %run over channels
        if (hd.recChDurations(ch) > hd.recChDurations(largestF(1))) %longer recored
            largestF(1) = ch;%number of file (in ncsFiles structure) with largest recordations time
        end
        if (hd.ch_si(ch) < hd.ch_si(largestF(2))) %smaller discretization time
            largestF(2) = ch;%number of file (in ncsFiles structure) with smallest discretization time
            %hd.si and hd.ch_si(largestF(2)) are equal
        end
    end
end
disp(flNm)

ii = hd.recChDurations > 0;%valid records only
if any(diff(hd.recChDurations(ii)) ~= 0)
    disp('Not equal samples number')
    %такое получается при разных частотах дискретизации
end
...
zavp.dwnSmplFrq = dwnSmplFrq;%1000;%downsampled-discretization frequency (Hz, for signals to be treated)
zavp.rarStep = NaN(1, hd.nADCNumChannels);%step of rarefy to get 1 ms descretezation (samples) (or simply rarefy step)
...
data = randn(round(hd.recChDurations(largestF(1)) / hd.ch_si(largestF(2))), 1);%pseudo data with maximal samples possible
%hd.si and hd.ch_si(largestF(2)) are equal

if (fs > zavp.dwnSmplFrq) %downsampling
    bfrr = resample(data, zavp.dwnSmplFrq, round(fs));%LFP resampled down to zavp.dwnSmplFrq Hz
else %upsampling
    tmOrig = (0:hd.si:((size(data, 1) - 1) * hd.si))';%original time
    tmResm = (0:(1e6 / zavp.dwnSmplFrq):((size(data, 1) - 1) * hd.si))';%resempled (upsampled) time
...
if muacr %common reference for MUA calculation
    ...
    for cg = 1:length(chnlGrp) %run over channel groups (shanks)
        realComRef = any(ismember(chnlGrp{cg}, chanMask));%any channel of the group contain spikes
...
%                 disp(['read channel ', num2str(ch), ' of ', num2str(hd.nADCNumChannels)])
                ...
                data = ZavRemovNaN(data);%exclude NaN and Inf with interpolation
                maxFlatLen = round(5e3 / hd.ch_si(ch)) + 1;%maximal acceptable length of flat segment (samples, 5ms)
                recLen = size(data, 1);%memory preallocation (length of single-sweep largest recordation)
                if (realComRef && ~exist('dataFltCmn', 'var') && (recLen > 0)) %any channel of the group contain spikes
                    dataFltCmn = NaN(recLen, length(chnlGrp{cg}), 'single');%filtered traces from all channels of current shank
                    mednCh = NaN(recLen, 1, 'single');%spatial median (over channels)
        %             dsprCh = zeros(recLen, 1, 'single');%spatial dispertion (over channels)
                end
                
                if (size(data, 1) > 1) %full length recordation
                    fs = 1e6 / hd.ch_si(ch);%sampling frequency (Hz)
                    if (fs > zavp.dwnSmplFrq) %downsampling
                        bfrr = resample(data, zavp.dwnSmplFrq, round(fs));%LFP resampled down to zavp.dwnSmplFrq Hz
                        lfp(1:numel(bfrr), ch, sw) = bfrr;%downsampled LFP
                    else %upsampling
                        tmOrig = (0:hd.ch_si(ch):((size(data, 1) - 1) * hd.ch_si(ch)))';%original time
                        tmResm = (0:(1e6 / zavp.dwnSmplFrq):((size(data, 1) - 1) * hd.ch_si(ch)))';%resempled (upsampled) time
                        bfrr = interp1(tmOrig, data, tmResm);%upsampled lfp
                        lfp(1:numel(bfrr), ch, sw) = bfrr;%upsampled LFP
                    end
                    %interval corresponding to 1(one) millisecond ((1e6 / zavp.dwnSmplFrq) microseconds)
                    zavp.rarStep(ch) = length(data) / length(bfrr);%step of rarefy to get 1 ms descretezation (samples) (or simply rarefy step)

                    [pntsL, pntsR] = ZavFindFlats(data, 1e-2);%flat segments
                    jj = (diff([pntsL, pntsR], 1, 2) <= maxFlatLen);%number of short segment (<1ms)
                    pntsL(jj) = []; pntsR(jj) = [];%1ms and longer silent segments only

                    if ((hd.ch_si(ch) * 1e-6) < 1e-3) %fast discretisation necessary
                        dataFlt = ZavFilter(data, fs, 'high', 300, 2);%highpass filtration
                        if ((fs / 2.001) > 4000) %fast discretisation
                            dataFlt = ZavFilter(dataFlt, fs, 'low', 4000, 2);%lowpass filtration
                        end
                    ...
                    lfpVar(ch, sw) = CalcMinVar(dataFlt, (hd.ch_si(ch) * 1e-6), data, 1);%lfp(:, ch, sw));%minimal dispers on a trace

                    [pntsL2, pntsR2] = ZavFindFlats(dataFlt, 1e-2);%flat segments
                    jj = (diff([pntsL2, pntsR2], 1, 2) <= maxFlatLen);%number of short segment (<1ms)
                    pntsL2(jj) = []; pntsR2(jj) = [];%1ms and longer silent segments only
                    pntsL = [pntsL(:); pntsL2(:)];
                    pntsR = [pntsR(:); pntsR2(:)];

                    if realComRef %any channel of the group contain spikes
...
%                 else %no record
%                     disp('skipped channel: no record')
                end
            end
            if ((length(chnlGrp{cg}) > 2) && realComRef) %many independent channels recorded and any channel of the group contain spikes
...
%             dspPrg = z + 4 * std(aa);%threshold for deviation of dispertion
            ...
            for nk = 1:length(chnlGrp{cg}) %run over channels in the group
                ch = chnlGrp{cg}(nk);%real number of channel
                if (ismember(ch, chanMask) && ((hd.ch_si(ch) * 1e-6) < 1e-3)) %only channels contaning EEG-signal (fast discretisation necessary)
%                     disp(['spikes of channel ', num2str(ch)])
                    ...
                    spks(ch, sw).tStamp = (spks(ch, sw).tStamp - 1) * hd.ch_si(ch) / 1e3;%moments of peaks of spikes appearance (ms from record beginning (from 0))
...
else %without common reference
    chnlGrp = {};%no channel groups
    disp('without common reference')
    for ch = 1:hd.nADCNumChannels %run over channels
        for sw = 1:hd.lActualEpisodes %run over sweeps (segments, episodes)
            data = ZavLoadData(flNm, hd, ch, sw, [], [], nlxVer);%read one channel all sweeps (no nlx-spikes)
            data = squeeze(data);%delete excess dimensions
            ...
            if (size(data, 1) > 1) %full length recordation
                ...
                if (fs > zavp.dwnSmplFrq) %downsampling
                    ...
                    tmOrig = (0:hd.ch_si(ch):((size(data, 1) - 1) * hd.ch_si(ch)))';%original time
                    tmResm = (0:(1e6 / zavp.dwnSmplFrq):((size(data, 1) - 1) * hd.ch_si(ch)))';%resempled (downsampled) time
                    bfrr = interp1(tmOrig, data, tmResm);%upsampled lfp
                    lfp(1:numel(bfrr), ch, sw) = bfrr;%upsampled LFP
                end
                %interval corresponding to 1(one) millisecond ((1e6 / zavp.dwnSmplFrq) microseconds)
...
if ~isfield(zavp, 'dwnSmplFrq') %field exist
    ...
    zavp.dwnSmplFrq = 1 / (zavp.siS * zavp.rarStep);%sampling frequency for rare data
end
...
for sw = 1:hd.lActualEpisodes
    for t = 1:numel(zavp.realStim(sw).r) %run over synchro-events
        segms(z, 1) = zavp.realStim(sw).r(t) / zavp.rarStep(1);%moment of synchro-event (ms from record begin (from 0))
        segms(z, 1) = zavp.realStim(sw).r(t) * zavp.siS * 1e3;%moment of synchro-event (ms from record begin (from 0))
...
% set(gcf, 'DefaultTextFontSize', 14, 'DefaultTextFontName', 'Arial', 'DefaultAxesFontSize', 14, 'DefaultAxesFontName', 'Arial');
% set(gcf, 'Units', 'normalized', 'Position', [0.003, 0.046, 0.45391, 0.3916]) %compact (1280x1024)
% %set(gcf, 'Units', 'pixels', 'Position', [5, 49, 581, 401]) %compact (pixels)
% % set(gcf, 'Units', 'normalized', 'Position', [0.003, 0.046, 1, 0.3916]) %horizontal
% %set(gcf, 'Units', 'pixels', 'Position', [5, 49, 1272, 401]) %horizontal (pixels)
% % set(gcf, 'Units', 'normalized', 'Position', [0.003, 0.046, 0.45391, 0.88379]) %vertical
% % set(gcf, 'Units', 'pixels', 'Position', [5, 105, 560, 905]) %vertical (pixels)
%
% % set(gcf, 'Units', 'normalized', 'Position', [1.003, 0.003, 1, 0.3916]) %horizontal (II monitor)
% % set(gcf, 'Units', 'normalized', 'Position', [1.003, 0.003, 0.45391, 0.88379]) %vertical (II monitor)
...
pltMUA = 1;%plot MUA if true

dCoeff(1) = -1.0;%polarity (scale
dCoeff(2) = 1000;%max(max(abs(whtLFP)));%max(max(abs(lfp(:, rCh((n + 1):end)))));%characteristic sizes (amplitude coefficient)
caRng = 50;%range of color axis for CSD %max(max(abs(lfpCSD)));

segmEdge = [-100, 1000] + 00;%left and right shifts from synchro-point (ms)
rCh = 0 + (1:1:16);%:16;%channels to be read and draw (1:hd.nADCNumChannels)
    %EEG: [30, 1, 5, 9, 13, 17, 21, 25]; %[30, 4, 8, 16, 20, 24, 27];
    %poly3: (1:10); %(11:22); %(23:32);
tecCh = 0;%number of special channels

sn = 1204;%(size(segms, 1) - 150):(size(segms, 1) - 50);%size(segms, 1);%wanted sweeps or segments
...
if rawData %raw data requested
    ...
    prm.Fs = zavp.dwnSmplFrq;%sampling frequency for downsampled data
end
...
whtLFP = whtLFP - repmat(median(whtLFP((tm >= -250) & (tm <= 0), :, :), 1), size(whtLFP, 1), 1);%remove DC
...
% %= end of (common reference) =%
    

%special channels
for z = 1:size(whtLFP, 3) %run over segments
    for ch = numel(rCh) + (0:-1:(1 - tecCh))
        whtLFP(:, ch, z) = (whtLFP(:, ch, z) - mean(whtLFP(:, ch, z))) * 0.5;%scalling of signal (piezo)
    end
end
...
for z = 1:size(whtLFP, 3) %run over segments
    ...
    %     whtLFP(:, ch, z) = whtLFP(:, ch, z) - (p1(1) * tm + p1(2));
        %1
    %     whtLFP(:, ch, z) = locdetrend(whtLFP(:, ch, z), 1e3, [300, 10]);
        %2
    %     whtLFP(:, ch, z) = ZavFilter(whtLFP(:, ch, z), prm.Fs, 'high', 300, 2);%high-pass
    %     whtLFP(:, ch, z) = ZavFilter(whtLFP(:, ch, z), prm.Fs, 'low', 5000, 2);%low-pass
    %     whtLFP(:, ch, z) = locdetrend(whtLFP(:, ch, z), prm.Fs, [0.002, 0.001]);%for raw data (spikes)
        %3
    %     whtLFP(:, ch, z) = ZavSpctSubtr(whtLFP(:, ch, z), []);%denoise by spectral subtraction

        %4 (RC-filter)
    %     whtLFP(:, ch, z) = ZavRCfilt(whtLFP(:, ch, z), 5, prm.Fs, 'high');
    
        %5 (DC-recovery)
    %    whtLFP(:, ch, z) = RecovDCfromPseudo(whtLFP(:, ch, z), zavp.dwnSmplFrq);%recovery
    end
end
...
% %     ajj = filtfilt([0.607, 1, 0.607], 2.213, ajj')';%spatial filtration    
    ...
    for z = 1:size(lfpCSD, 3) %run over segments
        for t = 1:size(lfpCSD, 2) %run over (channels) depth
            lfpCSD(:, t, z) = smooth(lfpCSD(:, t, z), k);%smoothing along time
        end
    end
    
    %spatial filtration
    for z = 1:size(lfpCSD, 3)
        lfpCSD(:, :, z) = filtfilt([0.607, 1, 0.607], 2.213, lfpCSD(:, :, z)')';%3-channel spatial smoothing as in elephy
    end
...
%     end
    ...
    lfpCSD = zeros(size(ajj, 1), numel(1:spPrc:((numel(rCh) - tecCh) - 2)), size(ajj, 3));
    for z = 1:size(lfpCSD, 3) %run over segments
        for t = 1:size(lfpCSD, 1) %run over time
            lfpCSD(t, :, z) = interp1(1:((numel(rCh) - tecCh) - 2), ajj(t, :, z), 1:spPrc:((numel(rCh) - tecCh) - 2));%interpolation along channels
            %lfpCSD(t, :) = spline(1:((numel(rCh) - spchn)- 2), ajj(t, :), 1:spprc:((numel(rCh) - spchn) - 2));%spline interpolation along channels
        end
    end
    
    lfpCSD = mean(lfpCSD, 3);%average by sweeps (segments)
...
if ((numel(sn) == 1) && pltMUA) %single sweep (segment)
    ...
    for ch = 1:numel(rCh) %run over wanted channels
        ...
        spkCntTrac = NaN(3 * length(spkDens), 2);%continuous trace of spikes
        spkCntTrac(1:3:end, 1) = spkDens;%time point of lower level
        spkCntTrac(2:3:end, 1) = spkDens;%time point of upper level
        spkCntTrac(1:3:end, 2) = ch - 0.3;%lower level
        spkCntTrac(2:3:end, 2) = ch - 0.1;%upper level
        plot(spkCntTrac(:, 1) / dCoeff(4), spkCntTrac(:, 2), 'r', 'LineWidth', 1.5) %fast drawing of spikes
    end
end
%= end of plot MUA =%

set(gca, 'YDir', 'reverse', 'XLim', segmEdge / dCoeff(4), 'YLim', [0, numel(rCh) + 1])
set(get(gca, 'XLabel'), 'String', ['time, ', bufStr])

%disp([num2str((segms(sn([1, end]), 1)' + segmEdge) / 1e3), ' s'])
if (isfield(hd, 'recChNames') && size(hd.recChNames, 2) > 1) %alternames was signed
...
title([fN, ' ', num2str(round(segms(sn(1), 1) + segmEdge(1)) / 1e3), '-', num2str(round(segms(sn(end), 1) + segmEdge(2)) / 1e3), ' s'])



%% === MUA density === %%
segmEdge = [-10, 60] + 0;%left and right shifts from synchro-point (ms)
dCoeff(1) = -1; dCoeff(2) = 3;%50;%max(max(spkDens));%characteristic sizes
caRng = 2.0;%0.3;%range of MUA colormap %max(max(spkDens));%
clr = 'w';%'w' - show colored spike density; any else - plot timecourses of spike density 

rCh = (1:16);%channels to be read and draw (1:hd.nADCNumChannels)
%EEG: [30, 1, 5, 9, 13, 17, 21, 25]; %[30, 4, 8, 16, 20, 24, 27];
%poly3: (1:10); %(11:22); %(23:32);

%2750:2950 %[1100:1111, 1118:1140] %1200:1350
sn = 301:400;%(size(segms, 1) - 50):(size(segms, 1) - 0);%:size(segms, 1)%wanted sweeps or segments
sn = sn(isfinite(segms(sn, 1)));
if isempty(sn), disp('no one segment choosed'); return; end

binStep = [5, 10];%[100, 5];%[bin size (ms), precision (must be integer)]
...
segmEdge = [0, 1.2e5];%left and right shifts from synchro-point (ms)
rCh = 6;%[3, 8, 14] channels to be read and draw
sn = 325;%450:600;%600:700;%250:350;%:size(segms, 1);%wanted sweeps or segments
sn = sn(isfinite(segms(sn, 1)));

whtLFP = ZavSynchLFP(zavp, hd, segms(sn, :), segmEdge, fLFP, rCh, rawData, nlxVer);%lfp phased with respect to synchro-events
% whtLFP = ZavSynchLFP(zavp, hd, segms(sn, :), segmEdge, lfp, rCh, rawData, nlxVer);%lfp phased with respect to stimulus moments
whtLFP = whtLFP - repmat(median(whtLFP(1:end, :, :), 1), size(whtLFP, 1), 1);%DC-shift
...
prm.fpass = [0.1, 100];%frequency range
prm.tapers = [diff(prm.fpass) * diff(segmEdge) * 1e-3, 5];%[diff(prm.fpass), diff(segmEdge) * 1e-3, 180];%[3, 5];%tapers 
...
% %3
% for t = 1:size(whtLFP, 2) %run over segments
...
% %4
% noisSpct = ((1:size(pwrSpct, 1))') .^ -1.8;%power low spectrum
% noisSpct = noisSpct * (max(pwrSpct) / max(noisSpct));
% pwrSpct = pwrSpct - noisSpct;%real part of spectrum after subtraction
% pwrSpct(pwrSpct < 0) = 0;

% %5
% [np, noisSpct] = ZavPowerAppr(log(frq'), log(pwrSpct));%spectrum subtraction
% pwrSpct = pwrSpct - exp(noisSpct);

% err(1, :) = smooth(err(1, :), 3);
% err(2, :) = smooth(err(2, :), 3);
% pwrSpct = smooth(pwrSpct, 3);

clf, hold on
%plot(repmat(frq, 2, 1), [pwrSpct' + err(1, :); pwrSpct' + err(2, :)], 'r', 'LineWidth', 2); hold on
ii1 = 1:3:length(frq); ii2 = length(frq):-3:1;
patch([frq(ii1), frq(ii2)], [pwrSpct(ii1)' + err(1, ii1), pwrSpct(ii2)' + err(2, ii2)], 'r', 'FaceColor', [1, 0, 0], 'EdgeColor', [1, 0, 0])
plot(frq(ii1), pwrSpct(ii1), 'k', 'LineWidth', 2)
set(gca, 'YScale', 'log')
set(gca, 'XScale', 'log')
...
prm.fpass = [8, 100];
prm.tapers = [2, 3];%tapers %[diff(prm.fpass), diff(segmEdge) * 1e-3, 1]
prm.pad = 2;%padding factors
prm.trialave = 1;%average over trials/channels when 1
...
segmEdge = [0, 300e3] + 0e3;%left and right shifts from synchro-point (ms)
rCh = 6;%channels to be read and draw
sn = 1;%:size(segms, 1);%wanted sweeps or segments
...
for ch = 1:numel(rCh)
    ...
    %whtLFP = ZavSynchLFP(zavp, hd, segms(sn, :), segmEdge, lfp, rCh(ch), rawData, nlxVer);%lfp phased with respect to stimulus moments
    whtLFP = ZavSynchLFP(zavp, hd, segms(sn, :), segmEdge, fLFP, rCh(ch), rawData, nlxVer);%lfp phased with respect to stimulus moments
...
rCh = et1(w).chAC;%1:16;%channels for LFP
sn = 1100:1380;%2800:3000;%:size(segms, 1);%wanted sweeps or segments

%fLFP = ZavFilter(lfp, prm.Fs, 'high', 2, 2);%filtered LFP
whtLFP = ZavSynchLFP(zavp, hd, segms(sn, :), segmEdge, fLFP, 1:length(rCh), rawData, nlxVer);%lfp phased with respect to synchro-events

tm = segmEdge(1):(1e3 / prm.Fs):segmEdge(2);%time matrix for raw or downsampled data (ms)
k0 = find(tm >= 0, 1, 'first');

binStep = [0.2, 5];%[bin size, precision (must be integer)]
[~, basbins] = ZavOverHist(rand(1e3, 1), binStep, [-2.2 * pi, 2.2 * pi]);
spkCircDens = zeros(length(basbins), length(rCh));
for ch = 1:length(rCh) %run over channels
    sw = 1;
    clear spkNew
    spkNew(1:length(sn)) = struct('s', [], 'ph', []);%spikes phases
    lfpPh = unwrap(angle(hilbert(-squeeze(whtLFP(:, 6, :)))));%hilbert phase
    for t = 1:length(sn) %run over segments
        lfpPh(:, t) = lfpPh(:, t) - lfpPh(k0, t);
        lfpPh((lfpPh(:, t) < (-3 * pi)) | (lfpPh(:, t) > (3 * pi)), t) = Inf;
        tW = segms(sn(t), 1) + segmEdge;
        spkNew(t).s = spks(rCh(ch), sw).tStamp(spks(rCh(ch), sw).tStamp > (tW(1) + 1) & spks(rCh(ch), sw).tStamp < (tW(2) - 1)) - tW(1);
        ii = round(spkNew(t).s);
        if ~isempty(ii)
            %ii((ii < 1) | (ii >= size(lfpPh, 1))) = [];
            spkNew(t).ph = lfpPh(ii, t);
        end
    end

    p1 = vertcat(spkNew(:).ph);
    p1 = p1(isfinite(p1));
    if (numel(p1) > 0)
        spkCircDens(:, ch) = ZavOverHist(p1, binStep, [-2.2 * pi, 2.2 * pi]);%histc(p1, bins);
    end
    
%     ii = (p1 > (-pi / 1)) & (p1 < (pi / 1));%one central cycle
%     %= circular parameters v1 =%
%     [p, z] = circ_rtest(p1(ii));
%     r = circ_r(p1(ii));%
%     th0 = circ_mean(p1(ii));%peak angle (rad)
%     %= end of (circular parameters v1) =%
% 
%     % %= circular parameters v2 =%
%     % [p, th0, r, z] = RayleighTest(p1(ii));%,clu,maxclu);
%     % z = exp(z);
%     % %= end of (circular parameters v2) =%
% 
%     r = round(r * 1e3) / 1e3;%rounding
%     p = round(p * 1e5) / 1e5;%rounding
%     th0 = 180 * th0 / pi;%peak angle (degree)
%     title(['r=', num2str(r), '; th0=', num2str(th0), '; p=', num2str(p), '; z=', num2str(z), '; n=', num2str(sum(ii))])
end
...
plot(basbins, (spkCircDens / dCoeff(3)) + repmat(1:numel(rCh), length(basbins), 1), 'r', 'LineWidth', 2)

xlim(2 * pi * [-1, 1])
set(gca, 'XTick', (-2 * pi):pi:(2 * pi), 'XTickLabel', {'-2p', '-p', '0', 'p', '2p'})
set(gca, 'YDir', 'reverse')
ylim([0.5, 16.5])
% polar(bins, histc(p1(ii), bins))
...
tm = (zavp.realStim.r - 1) / (zavp.rarStep * 1e3);%actual time (sec)
...
if isStim %try to find synchro impulses
    ...
    elseif (isequal(hd.fFileSignature(1:3), 'ABF') || isequal(hd.fFileSignature, 'DAQ')) %ABF or DAQ
        if isequal(hd.fFileSignature(1:3), 'ABF') %ABF
            data = ZavLoadData(flNm, hd, 'a', 1, 0, 'e', []);%min(120, diff(hd.recTime)));%one sweep (segment) for stimulus channel definition
        else %DAQ
            if ((hd.ObjInfo.SamplesPerTrigger == Inf) || isequal(hd.ObjInfo.SamplesPerTrigger, 'Inf')) %gap free
                data = ZavLoadData(flNm, hd, 'a', min(2, hd.lActualEpisodes), 0, min(20, diff(hd.recTime)), []);%one segment for stimulus channel definition
            else %sweeps
                data = ZavLoadData(flNm, hd, 'a', min(2, hd.lActualEpisodes), 0, min(20, 2 * diff(hd.recTime) / hd.lActualEpisodes), []);%one sweep for stimulus channel definition
            end
...
%                 end
        ...
        rws = rws(:, isfinite(rws(4, :)));
        [~, t] = max(rws(4, :));
        stimCh = rws(5, t);%channel with stimulus artefact 

%         %= special for daq of Marseille_2017 =%
%         stimCh = 1;%special for daq of Marseille_2017
%         %= end of (special for daq of Marseille_2017) =%
