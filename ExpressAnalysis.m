...
% Last Modified by GUIDE v2.5 07-Apr-2021 14:13:27
...
if (length(scanDrct) > 1)
    setappdata(expAn, 'scanDrct', scanDrct);
end

%define slash
z = find(scanDrct == '\', 1, 'last');%find slash
if isempty(z)
    slashStr = '/';%Linux mode slash
else
    slashStr = '\';%Windows mode slash
end
setappdata(expAn, 'slashStr', slashStr);%slash string
scanDrct = [scanDrct, slashStr];
set(expAn, 'Name', ['Express analysis - ', scanDrct]);%full name
...
for t = 3:length(dirCont) %run over directory content
    if (dirCont(t).isdir) %directory to be scanned
        if exist([scanDrct, dirCont(t).name, slashStr, 'Events.nev'], 'file') %Neuralynx
            fBase(w).srcFile = [scanDrct, dirCont(t).name, slashStr];%NLX
            ...
            for z = 3:length(subDirCnt) %run over objects of current folder
                if (~subDirCnt(z).isdir && (length(subDirCnt(z).name) > 3)) %file with quite long name
                    if (isequal(subDirCnt(z).name((end - 3):end), '.ncs') && ~isempty(strfind(subDirCnt(z).name, 'C'))) %ncs channel (LFP trace, "CSC" or "PxCxx" and so on)
            ...
            fclose(fid);
            bufStr = '.cfg';%'NexusConfigs\';%'Configuration\';%pattern to find
            cfgIn = strfind(clgfl, bufStr);%starting indices of config file extensions
            if ~isempty(cfgIn) %subdirectory exist
                bufStr = clgfl(cfgIn(end) + (-200:3));%part of configfile pathname
                z = strfind(bufStr, slashStr);
                cfgfl = bufStr((z(end) + 1):end);
                disp(['detected config-file: ', cfgfl])
                ...
                elseif isequal(cfgfl, 'A1x16_Brd2_double_4piezo.cfg')
                    fBase(w).chnlGrp(1) = {1:16}; fBase(w).chnlGrp(2) = {17:32}; fBase(w).chnlGrp(3) = {33:36};
                elseif isequal(cfgfl, 'A1x16_Brd1_double_4piezo_DCdivid.cfg')
                    fBase(w).chnlGrp(1) = {1:16}; fBase(w).chnlGrp(2) = {17:32}; fBase(w).chnlGrp(3) = {33:37};
                elseif isequal(cfgfl, 'A4x8_Brd1_double_4piezo_DCdivid.cfg')
                    fBase(w).chnlGrp(1) = {1:8}; fBase(w).chnlGrp(2) = {9:16}; fBase(w).chnlGrp(3) = {17:24}; fBase(w).chnlGrp(4) = {25:32};
                    fBase(w).chnlGrp(5) = {33:40}; fBase(w).chnlGrp(6) = {41:48}; fBase(w).chnlGrp(7) = {49:56}; fBase(w).chnlGrp(8) = {57:64};
                    fBase(w).chnlGrp(9) = {65:69};
                elseif isequal(cfgfl, 'A4x8_Brd1_double_4piezo_pseudoDC.cfg')
                    fBase(w).chnlGrp(1) = {1:8}; fBase(w).chnlGrp(2) = {9:16}; fBase(w).chnlGrp(3) = {17:24}; fBase(w).chnlGrp(4) = {25:32};
                    fBase(w).chnlGrp(5) = {33:40}; fBase(w).chnlGrp(6) = {41:48}; fBase(w).chnlGrp(7) = {49:56}; fBase(w).chnlGrp(8) = {57:64};
                    fBase(w).chnlGrp(9) = {65:68};
                elseif isequal(cfgfl, 'A1x16_Brd1_A1x16_Brd2_4piezo_DC_DC.cfg')
                    fBase(w).chnlGrp(1) = {1:16}; fBase(w).chnlGrp(2) = {17:32}; fBase(w).chnlGrp(3) = {33:48}; fBase(w).chnlGrp(4) = {49:64};
                    fBase(w).chnlGrp(5) = {65:69};
                elseif isequal(cfgfl, 'A1x16_Brd1_A1x16_Brd3_4piezo_AC_DC_Dasha.cfg')
                    fBase(w).chnlGrp(1) = {1:16}; fBase(w).chnlGrp(2) = {17:32}; fBase(w).chnlGrp(3) = {33:48}; fBase(w).chnlGrp(4) = {49:64};
                    fBase(w).chnlGrp(5) = {65:69};
                elseif isequal(cfgfl, 'A8x8_Brd1-2_doubled_4piezo_pseudoDC_AC.cfg')
                    fBase(w).chnlGrp(1) = {1:8}; fBase(w).chnlGrp(2) = {9:16}; fBase(w).chnlGrp(3) = {17:24}; fBase(w).chnlGrp(4) = {25:32};
                    fBase(w).chnlGrp(5) = {33:40}; fBase(w).chnlGrp(6) = {41:48}; fBase(w).chnlGrp(7) = {49:56}; fBase(w).chnlGrp(8) = {57:64};
                    fBase(w).chnlGrp(9) = {65:72}; fBase(w).chnlGrp(10) = {73:80}; fBase(w).chnlGrp(11) = {81:88}; fBase(w).chnlGrp(12) = {89:96};
                    fBase(w).chnlGrp(13) = {97:104}; fBase(w).chnlGrp(14) = {105:112}; fBase(w).chnlGrp(15) = {113:120}; fBase(w).chnlGrp(16) = {121:128};
                    fBase(w).chnlGrp(17) = {129:132};
                elseif isequal(cfgfl, 'A1x16_Brd1_glass_Brd2_double_4piezo_DCdivid.cfg')
                    fBase(w).chnlGrp(1) = {1:16}; fBase(w).chnlGrp(2) = {33};  fBase(w).chnlGrp(3) = {17:32}; fBase(w).chnlGrp(4) = {34};
                    fBase(w).chnlGrp(5) = {35:39};
                end
            end
            ii(w) = t;%files order
            w = w + 1;%next file
        elseif exist([scanDrct, dirCont(t).name, slashStr, 'structure.oebin'], 'file') %OpenEphys
            fBase(w).srcFile = [scanDrct, dirCont(t).name, slashStr];%OpenEphys
            fBase(w).type = 'OEbin';%type of file
            oeHd = load_open_ephys_binary([fBase(w).srcFile, slashStr, 'structure.oebin'], 'header', 1);%get OpenEphys header
            fBase(w).cscN = oeHd.Header.num_channels;%channels number
            ii(w) = t;%files order
            w = w + 1;%next file
        end
    elseif ((length(dirCont(t).name) > 3) && isequal(dirCont(t).name((end - 3):end), '.daq')) %daq
        fBase(w).srcFile = [scanDrct, dirCont(t).name];%daq
        daqHd = daqread(fBase(w).srcFile, 'info');%recordation attributes
        fBase(w).type = 'DAQ';%type of file
        fBase(w).cscN = length(daqHd.ObjInfo.Channel);%number of LFP channels
        ii(w) = t;%files order
        w = w + 1;%next file
    elseif ((length(dirCont(t).name) > 3) && isequal(dirCont(t).name((end - 3):end), '.abf')) %abf
        fBase(w).srcFile = [scanDrct, dirCont(t).name];%abf
        [~, ~, abfHd] = abfload(fBase(w).srcFile, 'channels', 'a', 'sweeps', 1, 'start', 0, 'stop', 0.1);%get header
        fBase(w).type = 'ABF';%type of file
        fBase(w).cscN = abfHd.nADCNumChannels;%number of LFP channels
        ii(w) = t;%files order
        w = w + 1;%next file
    end
end
...
for t = 1:length(fBase)
    ...
    if ~isempty(fBase(t).srcFile)
        if (isequal(fBase(t).type, 'NLX') || isequal(fBase(t).type, 'OEbin'))
            k = find(fBase(t).srcFile(1:(end - 2)) == slashStr, 1, 'last') + 1;
        else
            k = find(fBase(t).srcFile == slashStr, 1, 'last') + 1;
        end
        ...
        k = find(fBase(t).matFile == slashStr, 1, 'last') + 1;
...
bbckClr = get(hObject, 'BackgroundColor');%background color
set(hObject, 'BackgroundColor', [1, 0, 0], 'String', 'On process')
drawnow
...
muacr = get(handles.MUAcomRef, 'Value');%mua common reference
slashStr = getappdata(expAn, 'slashStr');%slash string

for rf = 1:length(fBase) %run over records
    if rList{rf, 2} %treat wanted files only
        if isempty(fBase(rf).matFile) %non converted data
            if isequal(fBase(rf).type, 'NLX') %NLX
                ...
                try
                    ZavSoursAnalys(fBase(rf).srcFile, 1, any(ttl > 0), [], 1:fBase(rf).cscN, fBase(rf).chnlGrp, nlxVer, muacr, 1000);%convertation
                    fBase(rf).matFile = [fBase(rf).srcFile(1:(end - 1)), '.mat'];%corresponding mat-file
                    rList{rf, 1} = [rList{rf, 1}, ' (+mat)'];%mat-file added
                    set(handles.recList, 'Data', rList);%renew visible list
                    drawnow
                catch errMess
                    set(hObject, 'BackgroundColor', bbckClr, 'String', 'Convert raw')
                    msgs = get(textMsgs, 'String');
                    msgs{end + 1} = ['error on ', fBase(rf).srcFile, ' :=:', errMess.message];
                    set(textMsgs, 'String', msgs);
                    disp(msgs{end})
                end
            elseif isequal(fBase(rf).type, 'OEbin') %OpenEphys
                try
                    isStim = true;%false;%evoked or spontaneous activity
                    ZavSoursAnalys([fBase(rf).srcFile, 'structure.oebin'], 1, isStim, [], 1:fBase(rf).cscN, [], 0, muacr, 1000);%convertation
                ...
                try
                    isStim = true;%false;%evoked or spontaneous activity
                    ZavSoursAnalys(fBase(rf).srcFile, 1, isStim, [], 1:fBase(rf).cscN, [], 0, muacr, 1000);%convertation
                    fBase(rf).matFile = [fBase(rf).srcFile(1:(end - 4)), '.mat'];%corresponding mat-file
                    rList{rf, 1} = [rList{rf, 1}, ' (+mat)'];%mat-file added
                    set(handles.recList, 'Data', rList);%renew visible list
                    drawnow
        ...
        if (isequal(get(handles.TreatAllRecords, 'State'), 'on') && ~isempty(fBase(rf).matFile)) %treat all records
            ...
            k = find(fBase(rf).matFile == slashStr, 1, 'last');
            ...
            if isempty(get(handles.PrincChan, 'String')) %request principal channel
                set(handles.PrincChan, 'String', '1') %default value
            end
            ...
            AnalyseEvoked(handles);%calculcate SEP parameters
        end
    end
end
set(hObject, 'BackgroundColor', bbckClr, 'String', 'Convert raw')
...
AnalyseEvoked(handles);%perform analysis



function AnalyseEvoked(handles)
...
if isequal(flNm((end - 3):end), '.mat')
    ...
    if ~isempty(zavp.realStim(1).r) %evoked signals (on-response)
        ...
        if treatAllCh %treat all channel
            fullChList = 1:hd.nADCNumChannels;%run over all LFP-channels
...
%             end
            ...
            [pntsL, pntsR] = ZavFindFlats(jj01, 1e-4);%flat segments
            ...
            if (~isempty(pntsL) && ~isempty(pntsR)) %at least one flat segment exist
                ...
                segmEdge = [p1 - 2, p2 + 2];%left and right shifts from synchro-point (ms)
...
%             end
            ...
            zavp.prm.Fs = 1 / (zavp.siS * zavp.rarStep(1));%sampling frequency for rare data
...
fullChList = 1:hd.nADCNumChannels;%run over all LFP-channels %rCh = getappdata(expAn, 'rCh');%principal channel
...
elseif isequal(get(hObject, 'Tag'), 'SkipSweep') %reject sweep
    sepOnsetPeak(rCh, sw).r(g, :) = Inf(size(sepOnsetPeak(rCh, sw).r(g, :)));%incalculable signal
    segmIndx = segmIndx + 1;%next segment
...
if ~isequal(new_rCh, rCh) %new channel
    AnalyseEvoked(handles);%start analysis again
end
...
if (onoff == 1) %on
    for sw = 1:hd.lActualEpisodes %run over sweeps
        for t = 1:numel(zavp.realStim(sw).r) %run over wanted synchro-events
            segms(z, 1) = zavp.realStim(sw).r(t) / zavp.rarStep(1);%position of synchro-event (ms from record begin (from 0))
    ...
    for sw = 1:hd.lActualEpisodes %run over sweeps
        for t = 1:numel(zavp.realStim(sw).f) %run over wanted synchro-events
            segms(z, 1) = zavp.realStim(sw).f(t) / zavp.rarStep(1);%position of synchro-event (ms from record begin (from 0))
            %segms(z, 1) = segms(z, 1);% + sepOnsetPeak(rCh, sw).r(z, 1);%shift to SEP onset
            segms(z, 2) = zavp.stimCh;%number of channel where syncro-event was detected
            segms(z, 3) = sw;%number of sweep where syncro-event was detected
            segms(z, 4) = t;%number of stimulus (in matrix zavp.realStim) or number of trough in brst matrix
...
nulTm = NaN(1, 3);
k = str2double(get(hourEdtH, 'String'));
if ~isempty(k)
    nulTm(1) = k;%start time (hours)
end
k = str2double(get(minEdtH, 'String'));
if ~isempty(k)
    nulTm(2) = k;%start time (minutes)
end
k = str2double(get(etalonEdtH, 'String'));
if ~isempty(k)
    nulTm(3) = k;%etalon record
end
...
if any(isnan(nulTm(1:2))) %no null time asigned
    t0 = 6 * 3600;
else
...
fullChList = 1:hd.nADCNumChannels;%run over all LFP-channels
% fullChList = str2double(get(handles.PrincChan, 'String'));%principal channel
prgMult = (-1) * str2double(get(handles.Porog, 'String'));%threshold multiplier

selRec = find(horzcat(rList{:, 2}));%selected files number (in fBase)
if ~isfinite(nulTm(3))
    nulTm(3) = selRec(1);%first selected
end
load(fBase(nulTm(3)).matFile, 'lfpVar')
etPrgs = lfpVar;%etalon thresholds

k = find(fBase(1).matFile == slashStr, 1, 'last');
z = find(fBase(1).matFile(1:(k - 2)) == slashStr, 1, 'last');
...
for rCh = fullChList %run over all channels
    paramToXLS = cell(5e3, 14);%memory preallocation
    paramToXLS(1, :) = {'dir', 'file', 'segment', 'sweep', 'channel', 'time of day', 'time after appl.', ...
              'SEP onset', 'SEP peak', 'SEP ampl', '0-20ms MUA', '20-520ms MUA', 'SEP slope', 'spnt MUA'};
    paramToXLS(2, :) = {'', '', '#', 'sw', 'ch', 'hh:mm:ss', 'seconds', ...
              'ms', 'ms', 'uV', 'units', 'units', 'uV/ms', 'unit/s'};
    px = 3;
    for w = selRec %run over selected records
        k = find(fBase(w).matFile(1:(end - 2)) == slashStr, 1, 'last');
        ...
        if any(ismember({'sepOnsetPeak', 'sepOnsetPeak2'}, varInFl))
            ...
            for z = 1:size(segms, 1) %run over segments
                segms(z, 5) = 0;%sweep start (seconds from beginning of recording)
                if isfield(hd, 'sweepStartInPts') %sweep start exist
                    segms(z, 5) = hd.sweepStartInPts(segms(z, 3)) * hd.si / 1e6;%sweep start (seconds from beginning of recording)
                end
                timeStr(z, :) = MoscowTime(hd.recTime(1) + segms(z, 5) + (segms(z, 1) * 1e-3));%"Moscow" time of the segment begin (part of day)
            end
            ...
            for z = 1:segmN %run over stimuli
                sw = segms(z, 3);%sweep
                if ~isempty(sepOnsetPeak2(rCh, sw).r)
                    stN = segms(z, 4);%number of stimulus is a sweep
                    sepOnsetPeak2(rCh, sw).r(stN, ~isfinite(sepOnsetPeak2(rCh, sw).r(stN, :))) = NaN;
                    if (z > 1)
                    ...
                    paramToXLS(px, :) = {fBase(w).matFile(1:k), fBase(w).matFile((k + 1):end), z, sw, rCh, timeStr(z, :), ... (dir, file, segment, sweep, channel)
                        round((hd.recTime(1) - t0 + segms(z, 5) + ((zavp.realStim(sw).r(stN) - 1) / zavp.rarStep(1)) * 1e-3) * 100) / 100, ... (time after appl.)
                        sepOnsetPeak2(rCh, sw).r(stN, 1), sepOnsetPeak2(rCh, sw).r(stN, 2), round(sepOnsetPeak2(rCh, sw).r(stN, 3) * 10) / 10, ... (SEP onset, SEP peak, SEP ampl)
                        sepOnsetPeak2(rCh, sw).r(stN, 4), sepOnsetPeak2(rCh, sw).r(stN, 5), round(sepOnsetPeak2(rCh, sw).r(stN, 6) * 10) / 10, ... (0-20ms MUA, 20-520ms MUA, SEP slope)
...
k = find(fBase(1).srcFile(1:(end - 2)) == slashStr, 1, 'last');
z = find(fBase(1).srcFile(1:(k - 2)) == slashStr, 1, 'last');
pf = [fBase(1).srcFile(1:k), 'graph', slashStr];%pathname for figure saving
...
%             %tm(t) = ((zavp.realStim.r - 1) / (zavp.rarStep(1) * 1e3 * 60)) + ((hd.recTime(1) / 60) - t0);
...
% %         %tm = ((zavp.realStim.r - 1) / (zavp.rarStep(1) * 1e3 * 60) + ((hd.recTime(1) / 60) - t0);
...
% --- Executes when ExpressAnalysis is resized.
function ExpressAnalysis_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to ExpressAnalysis (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

listHndl = handles.recList;%handle of table with recordation list
rList = get(listHndl, 'Data');%list of wanted files
mwpsn = get(handles.ExpressAnalysis, 'Position');%position of main window
tblpsn = get(listHndl, 'Position');%position of table
set(listHndl, 'ColumnWidth', {round(mwpsn(3) * tblpsn(3)) - 55, 35})
