function filteredS = ZavFilter(s, dF, filtTp, fStop, metod)
...
if (metod == 1)%single-step filtering
    [b1, a1] = cheby2(4, 20, fStop / (0.5 * dF), filtTp);%Chebyshev filter
elseif (metod == 2) %two-cascade filtering (Chebyshev)
    [b1, a1] = cheby2(4, 8, fStop / (0.5 * dF), filtTp);%first step Chebyshev filter
    [b2, a2] = cheby2(5, 8, fStop / (0.5 * dF), filtTp);%second step Chebyshev filter
elseif (metod == 3) %tree-cascade filtering (Chebyshev)
    [b1, a1] = cheby2(4, 8, fStop / (0.5 * dF), filtTp);%first step Chebyshev filter
    [b2, a2] = cheby2(5, 8, fStop / (0.5 * dF), filtTp);%second step Chebyshev filter
    [b3, a3] = cheby2(5, 8, fStop / (0.5 * dF), filtTp);%third step Chebyshev filter
elseif (metod == 4) %Butterworth filter
    [b1, a1] = butter(4, fStop / (0.5 * dF), filtTp);%Butterworth filter
    metod = 1;
else
