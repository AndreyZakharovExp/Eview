...
if (nargout > 1) %header requested
    ...
    hd.ch_si = hd.si * ones(hd.nADCNumChannels, 1);%sample interval (mks)
    ...
    hd.sweepLengthInPts = hd.ObjInfo.SamplesAcquired;%SamplesPerTrigger;
    hd.recChDurations = hd.ObjInfo.TriggersExecuted * hd.ObjInfo.SamplesAcquired * hd.si * ones(hd.nADCNumChannels, 1);%duration of the record (microseconds)
    hd.dataPtsPerChan = hd.ObjInfo.TriggersExecuted * hd.ObjInfo.SamplesAcquired * ones(hd.nADCNumChannels, 1);%samples per channel
end
